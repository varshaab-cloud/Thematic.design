...
Button renders a native `<button>` element. Passing `asChild` swaps it for Radix's `Slot`, which merges all button styling and props onto whatever single child you provide — use this to render a button-styled `<a>` or a Next.js `<Link>` without nesting interactive elements.

**What the component owns**

- **The loading spinner.** When `isLoading` is `true`, Button renders a `Loader2` icon from `lucide-react` with a spin animation before the label. The label stays visible — the spinner is prepended, not swapped in. Do not pass your own spinner.
- **Disabling during load.** `isLoading` sets `disabled` automatically. There is no need to pass both.
- **Icon sizing and spacing.** Any `svg` child is sized to match the button size (16px default, 12px at `xs`, 14px at `sm`) and given `pointer-events-none` and `shrink-0`. Do not set a size class on the icon; the component handles it.
- **Styling hooks.** Button emits `data-slot="button"`, `data-variant`, and `data-size` so parent components can target it without class-name coupling.

**What the consumer provides**

- **Icons**, as children from `lucide-react`. Mark them with `data-icon="inline-start"` or `data-icon="inline-end"` so the component can tighten the padding on that side.
- **The label**, as a text child. For icon-only buttons, pass the icon plus a `<span className="sr-only">` label, and use an `icon` size variant.
- **Grouping.** Wrapping buttons in a container with `data-slot="button-group"` adjusts the inner corner radii so the set reads as one connected control.
