...
/* The highlight answers "where does this token's value come from, and who uses it" —
   the path THROUGH the clicked token. Descendants: what it resolves to. Ancestors: what
   resolves to it. Crucially NOT siblings: clicking bg-checked must not light
   bg-indeterminate just because both use the same alias. (Click the alias itself to see
   everything that shares it — that is when siblings are the answer.) */
